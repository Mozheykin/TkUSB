...
def change_checkbutton_position(root, name:str, _type:str) -> None:
    ...
    match type_object.get(name)[1]:
        case 'yes':
            img = get_picture('check-no', 6, 6)
            actuall_button = 'no'
        case 'no':
            img = get_picture('check-null', 6, 6)
            actuall_button = 'null'
        case 'null':
            actuall_button = 'yes'
            img = get_picture('check-yes', 6, 6)
    ...
    createObjects.CreateObjects.Objects[_type][name] = [img_button, actuall_button]
